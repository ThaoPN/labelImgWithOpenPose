...
from libs.shape_pose import ShapePose, DEFAULT_LINE_COLOR, DEFAULT_FILL_COLOR
from libs.canvas_pose import CanvasPose
...
__appname__ = 'labelImg Edition load keypoints from txt'
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        self.canvas = CanvasPose(parent=self)
        ...
        poseEstimate = action('&Pose Estimate', self.poseEstimate, 'Ctrl+P', 'fit-window', u'Pose Estimate From CMU-COCO Model')
        ...
        '''self.actions.beginner = (poseEstimate,
        ...
        '''self.actions.advanced = (poseEstimate,
        ...
        if unicodeFilePath and os.path.exists(unicodeFilePath):
            ...
            '''if self.defaultSaveDir is not None:
                ...
                if os.path.isfile(xmlPath):
                    ...
                    self.loadYOLOTXTByFilename(txtPath)'''
            # THAOPN: load txt file here
            txtPath = self.getTXTFileFromImage(self.filePath)
            if txtPath is not None:
                self.loadPoseFromTXT(txtPath)
    ...
    def mayContinue(self):
        return True
        #THAOPN: prevent the app show dialog to ask save 
    ...
    def getTXTFileFromImage(self, imgPath):
        dirPathContaint = os.path.split(os.path.dirname(imgPath))[0]
        
        nameWithoutExt = os.path.basename(os.path.splitext(imgPath)[0])
        
        dirPathSavedKeypoints = os.path.join(dirPathContaint, "keypoints")
        if not os.path.isdir(dirPathSavedKeypoints):
            self.errorMessage(u'No txt loaded',
                    u'Your keypoints directory is not exists.')
            return None          
        
        keypointsFileName = os.path.join(dirPathSavedKeypoints, nameWithoutExt) + '.txt'
        
        return keypointsFileName
    
    def loadPoseFromTXT(self, txtPath):
